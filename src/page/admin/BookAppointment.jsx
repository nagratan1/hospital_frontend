...
Thank you!
    `.trim();
    const smsUrl = `sms:${data.mobile}?body=${encodeURIComponent(messageSMS)}`;
    window.location.href = smsUrl;
  };
  ...
  const handleDoctorChange = (e) => {
    ...
    // Always update formData doctorId (clears when blank)
    setFormData(prev => ({
      ...prev,
      doctorId: doctorId || '',
      booking_amount: doctor ? (doctor.consultationFee || '') : ''
    }));
    ...
    if (doctor) {
      ...
      // Clear availability if no doctor selected
      setAvailableDates([]);
      setSelectedDate('');
      setAvailableSlots([]);
      setSelectedSlot('');
  ...
  const handleChange = (e) => {
    ...
    // Clear form level error when the user edits fields
    if (formError) {
      setFormError('');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("Submitting appointment", { formData, selectedDate, selectedSlot, availableSlots });
    ...
    try {
      ...
      console.log("Dispatching AppointmentCreate", appointmentData);
      const response = await dispatch(AppointmentCreate(appointmentData));

      console.log("AppointmentCreate response", response);
    ...
    <Dashboard>
      ...
      <div className="h-screen flex flex-col bg-gray-50">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {formError && (
            <div className="mb-4 p-3 bg-yellow-50 border-l-4 border-yellow-500 text-yellow-700 rounded flex items-center text-sm">
              <svg className="h-4 w-4 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                <path d="M8.257 3.099c.765-1.36 2.681-1.36 3.446 0l6.516 11.583A1.75 1.75 0 0116.516 17H3.484a1.75 1.75 0 01-1.703-2.318L8.257 3.1zM11 14a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-.993.883L9 6v4a1 1 0 002 0V6a1 1 0 00-1-1z" />
              </svg>
              <span>{formError}</span>
            </div>
          )}
                        ...
                        {availableSlots.map((slot) => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              // keep formData in sync with chosen slot so validate/submit won't miss it
                              setFormData(prev => ({
                                ...prev,
